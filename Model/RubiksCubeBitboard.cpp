//
// Created by Aryaman Sahu on 28-06-2026.
//
...
private:
    ...
    void rotateFace(int ind) {
        ...
        side= side >> (8 * 6);
        bitboard[ind]= (bitboard[ind]<<(2*8)) | side;
    }
...
public:
    ...
    COLOR getColor(FACE face, int row, int col) const override {
        int idx=arr[row][col];
        if (idx==8) {
            return static_cast<COLOR>(static_cast<int>(face));
        }
    ...
    void setColor(FACE face, int row, int col, COLOR color) override {
        int idx=arr[row][col];
        if (idx==8) return;
        uint64_t side=static_cast<int>(face);
        bitboard[side] &= ~( (one_8) << (8*idx) );
        uint64_t newColor= (1ULL<<(static_cast<int>(color)));
        bitboard[side] |= (newColor<<(8*idx));
    };

    bool isSolved() const override {
        for (int side=0;side<6;side++) {
            if (bitboard[side]!=solved_side_config[side]) return false;
        }
        return true;
    };
