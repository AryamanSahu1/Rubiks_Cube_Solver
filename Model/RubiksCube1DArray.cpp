//
// Created by Aryaman on 27-06-2026.
//
...
public:
    ...
    void setColor(FACE face, int row, int col, COLOR color) override {
        cube[getIndex(static_cast<int>(face),row,col)]=getColorLetter(color);
    }
    ...
    bool isSolved() const override {
        for (int i=0;i<6;i++) {
            for (int j=0;j<3;j++) {
                for (int k=0;k<3;k++) {
                    if (getColorLetter(static_cast<COLOR>(i))!=this->cube[getIndex(i,j,k)]) {
                        return false;
                    }
                }
            }
