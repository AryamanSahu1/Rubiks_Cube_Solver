...
const map<GenericRubiksCube::COLOR, Scalar> CubeScanner::colorMap = {
    {GenericRubiksCube::COLOR::WHITE,  Scalar(255, 255, 255)},
    {GenericRubiksCube::COLOR::RED,    Scalar(0, 0, 255)},
    {GenericRubiksCube::COLOR::ORANGE, Scalar(0, 165, 255)},
    {GenericRubiksCube::COLOR::YELLOW, Scalar(0, 255, 255)},
    {GenericRubiksCube::COLOR::GREEN,  Scalar(0, 255, 0)},
    {GenericRubiksCube::COLOR::BLUE,   Scalar(255, 0, 0)},
    {GenericRubiksCube::COLOR::UNKNOWN, Scalar(50, 50, 50)}
};
...
GenericRubiksCube::COLOR CubeScanner::classifyColor(const Vec3b& bgr) {
    ...
    if (
        ...
        abs(bgr[1] - bgr[0]) < 30 &&
        abs(bgr[0] - bgr[2]) < 30
    ) return GenericRubiksCube::COLOR::WHITE;

    if (h >= 160 && h <= 190) return GenericRubiksCube::COLOR::RED;
    if (h >= 3 && h <= 19)   return GenericRubiksCube::COLOR::ORANGE;
    if (h >= 20 && h <= 30)  return GenericRubiksCube::COLOR::YELLOW;
    if (h >= 60 && h <= 90)  return GenericRubiksCube::COLOR::GREEN;
    if (h >= 100 && h <= 120)return GenericRubiksCube::COLOR::BLUE;
...
vector<vector<GenericRubiksCube::COLOR>> CubeScanner::captureFace() {
    Mat frame;
    int rows, cols;
    ...
    vector<vector<GenericRubiksCube::COLOR>> face(3, vector<GenericRubiksCube::COLOR>(3));
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j) {
...
Mat CubeScanner::drawColorFace(const vector<vector<GenericRubiksCube::COLOR>>& faceGrid) {
...
Mat CubeScanner::drawFullCube(const vector<vector<vector<GenericRubiksCube::COLOR>>>& cubeGrid) {
...
void CubeScanner::scan(GenericRubiksCube& cube) {
    vector<vector<vector<GenericRubiksCube::COLOR>>> cubeGrid(6, vector<vector<GenericRubiksCube::COLOR>>(3, vector<GenericRubiksCube::COLOR>(3, GenericRubiksCube::COLOR::WHITE)));
    ...
    for (int face = 0; face < 6; face++) {
        while (true) {
            ...
            if (key == 'n' || key == 'N') {
                for (int i = 0; i < 3; ++i)
                    for (int j = 0; j < 3; ++j)
                        cube.setColor(static_cast<GenericRubiksCube::FACE>(face), i, j, grid[i][j]);
